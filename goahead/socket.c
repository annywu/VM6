...
/*
    Edit on Nov.24th.  Write login IP Addr To File to solve Internet Problems.
 */

#define CLIENT_IP_FILE1 "/opt/cfg/client_ip1.cfg"
#define CLIENT_IP_FILE2 "/opt/cfg/client_ip2.cfg"

static void writeIpToFile(char* ipbuf)
{
    printf("[smvwebsvr print] : %s\n", ipbuf);
    static int write_flag = 0;
    char buf1[128] = {0};
    int fd1 = open(CLIENT_IP_FILE1, O_RDWR);
    read(fd1, buf1, 128);
    printf("[smvwebsvr print] : buf1 = %s\n", buf1);

    char buf2[128] = {0};
    int fd2 = open(CLIENT_IP_FILE2, O_RDWR);
    read(fd2, buf2, 128);
    printf("[smvwebsvr print] : buf2 = %s\n", buf2);

    if (!strcmp(ipbuf, buf1) || !strcmp(ipbuf, buf2))
    {
        printf("[smvwebsvr print] : notice !!! ip exits \n");
        return NULL;
    }
    else
    {
        if (!strlen(buf1) || 1 == write_flag || 0 == write_flag)
        {
            ftruncate(fd1, 0);
            lseek(fd1, 0, SEEK_SET);
            write(fd1, ipbuf, strlen(ipbuf));
            write_flag = 2;
        }
        else if (!strlen(buf2) || 2 == write_flag)
        {
            ftruncate(fd2, 0);
            lseek(fd2, 0, SEEK_SET);
            write(fd2, ipbuf, strlen(ipbuf));
            write_flag = 1;  
        }
