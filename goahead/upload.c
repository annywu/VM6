    ...
    if (scaselesscmp(headerTok, "Content-Disposition") == 0) {
        ...
        while (key && stok(key, ";\r\n", &nextPair)) {
            ...
            if (scaselesscmp(key, "form-data") == 0) {
                ...
                #if 1
                if (*value == '.' || !websValidUriChars(value) || strpbrk(value, "/*?()<>|~\"'%`^\n\r\t\f")) {//add "(" ")" on july 28th 2020.  del \\ : on Dec. 15th
                    websError(wp, HTTP_CODE_INTERNAL_SERVER_ERROR, "Bad upload client filename");
                    websWrite(wp, "Bad upload client filename");
                    printf("HTTP_CODE_INTERNAL_SERVER_ERROR Bad upload client filename\n");
                    wfree(value);
                    return;
                }
                #endif
                ...
                if ((wp->uploadTmp = websTempFile(uploadDir, "tmp")) == 0) {
                    ...
                    printf("Cannot create upload temp file %s\n. Check upload temp dir %s\n", wp->uploadTmp, uploadDir);
                    return;
                }
